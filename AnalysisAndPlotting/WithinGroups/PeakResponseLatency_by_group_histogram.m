function PeakResponseLatency_by_group_histogram(all_data,trialTagsLabels,trialType)
    ...
    for mouseNum = 1:length(mouseNames)
        ...
        for cellID_num = 1:length(cellIDs)
            ...
            if (responsivityNum == 1) && isSingleUnit % unit is positively modulated by stim
                ...
                FSL_vec(end+1,1) = all_data.(groupName).(mouseName).(cellID).PeakEvokedFR_Latency(trialTypeInd);
...
for cellTypeNum = 1:length(cell_types)
    ...
    hold on
    h1 = histogram(FSL_vec_cellType(strcmp(groupVec_cellType,groupNames{1})), 'BinWidth',2, 'FaceColor','red');
    h2 = histogram(FSL_vec_cellType(strcmp(groupVec_cellType,groupNames{2})), 'BinWidth',2, 'FaceColor','blue');
    ...
    xlabel('Peak Response Latency (ms)');
    ylabel('Unit Count');
end
