function AUROC_histogram(all_data)
...
for groupNum = 1:length(groupNames)
    ...
    for mouseNum = 1:length(mouseNames)
        ...
        for cellID_num = 1:length(cellIDs)
            ...
            AUROC_vec(end+1,1) = all_data.(groupName).(mouseName).(cellID).StimProb(1);
...
for cellTypeNum = 1:length(cell_types)
    AUROC_vec_cellType = AUROC_vec(strcmp(cellTypesVec, cell_types{cellTypeNum}));
    responsivityVec_cellType = responsivityVec(strcmp(cellTypesVec, cell_types{cellTypeNum}));

    t = nexttile(T);
    title(t, cell_types{cellTypeNum});
    hold on
    h1 = histogram(AUROC_vec_cellType(~strcmp(responsivityVec_cellType,'NR')), 25, 'BinLimits', [0 1]);
    h2 = histogram(AUROC_vec_cellType(strcmp(responsivityVec_cellType,'NR')), 25, 'BinLimits', [0 1]);
    ...
    xlabel('Stimulus Probability (AUROC)');
    ylabel('Unit Count');
end
