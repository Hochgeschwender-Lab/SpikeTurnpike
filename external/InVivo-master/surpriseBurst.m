...
burst.proportion_time_in_bursts = sum(burst.burst_duration) / T; % Grant - corrected this: sum of burst durations / recording duration (seconds)
% burst.proportion_spikes_in_bursts = burst.total_spikes_in_bursts/length(ISI(beg_idx+1:length(ISI)));
burst.proportion_spikes_in_bursts = burst.total_spikes_in_bursts/length(ts(ts >= local_length)); % Grant - simplified for sanity check

% mean/median burst duration added by Grant Folkert (7/7/2023)
burst.mean_burst_duration = mean(burst.burst_duration);
burst.median_burst_duration = median(burst.burst_duration);
