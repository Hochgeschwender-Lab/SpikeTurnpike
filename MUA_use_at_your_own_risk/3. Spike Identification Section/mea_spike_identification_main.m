...
for ii = 1:numfolders
    ...
    for n_folders=1:total_number_of_files_to_extract_from
        ...
        electrode_data_reordered = electrode_data_reordered.electrode_data_downsampled;
        ...
        sampling_frequency = 10e3; % Hz % Downsampled from 30kHz, currently 10kHz
        ...
        fprintf('\nBinning the spikes in %d ms bins for electrode observations...\n', bin_spikes_every_x_millisecond)
               ...
               % Bin spikes for further data analysis
        % Make time bins for every 'x' second
        bin_spikes_every_x_microsecond = 1; % ms
       
        % Get the "spike_density" data from the "spike_density_fun" function
        fprintf('Binning the spikes in %d us bins for further MUA analysis...\n', bin_spikes_every_x_microsecond)
        
        spike_density_for_analyzing_change_us = spike_density_us_fun(electrode_spike_identification, ...
             bin_spikes_every_x_microsecond, file_directory_to_save_data);
    
        % Save the "spike_density_for_analyzing_change" as a .mat data.
        fprintf("Saving spike density data for further analysis...\n")
        save(strcat(file_directory_to_save_data, 'allData/spike_density_for_analyzing_change_us.mat'),...
            'spike_density_for_analyzing_change_us', '-v7.3')
